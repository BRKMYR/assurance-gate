...
- Hazard mapping: categories are normalised on both sides (lowercase, punctuation to underscores) because strong_reject labels read "Non-violent crimes" and xstest unsafe prompts carry a `contrast_` prefix. The config names were aligned to the benchmark labels and a `privacy` hazard added. `self_harm` has no source in either benchmark and stays empty, so the coverage gate fails on it for every model. That is the intended reading of an empty hazard.
- Track B run 2 (2026-09-19 22:04Z to 22:30Z): all nine evaluations succeeded, 763 samples per model. Verdicts against the frozen gates: Qwen2.5-7B-Instruct NO GO on harmful compliance (55 of 296 scorable, upper bound 0.2349 against 0.05) and hazard coverage. Llama-3.1-8B-Instruct NO GO on over refusal by the bound only (20 of 250, point 0.080, upper bound 0.1209 against 0.10) and hazard coverage. gpt-oss-20b NO GO on over refusal (26 of 250, 0.104) and hazard coverage. `self_harm` is the empty hazard for all three.
- The build reads `configs/track_b.yaml` for hazards, builds the Track B coverage matrix and the sample level comparison against the configured baseline, and sets `bundle` to `inspect/index.html` when the viewer bundle is present in the output directory.
- CI: the stealth step calls the linter with its default paths. Passing `runs` and `site` as explicit paths applied the prose rules to model completions inside the sample records, which are data. The stealth grep still covers those directories in the default run.

## Coordinator, website as the primary page (2026-09-20)
- Owner decision: the whole project lives at brkmyr.com/gate/ as one page, no pages beneath it. The dashboard runs inside that page from local data, thumbnails and font, with the case study text (problem, solution, result, criteria, metrics) rendered around it. Hash views are not pages. The Hugging Face Space stays as the mirror and hosts the 26 MB Inspect viewer bundle, which the sample deep links point at. `scripts/publish_website.py` writes the page and its assets into the website repo.
